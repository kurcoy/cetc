...
volatile unsigned long long samples = 0;//uint32_t 
...
uint8_t DiffManchester_GetData( uint8_t data[CODE_BYTELENGTH], uint8_t dataLens )
{
  uint8_t  error=0;
  uint8_t i;
  unsigned long long rawSamps; //uint32_t
